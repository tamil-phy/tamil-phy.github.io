        ...
        try {
            ...
            markdownContent.querySelectorAll('img').forEach(img => {
                ...
                if (src) {
                    ...
                    if (src.includes('/images/') && (src.startsWith('/Users/') || src.startsWith('/home/') || src.startsWith('/c:/') || src.startsWith('C:\\'))) {
                        const filename = src.split('/images/').pop();
                        const newSrc = 'images/' + filename;
                        img.setAttribute('src', newSrc);
                        console.log('Fixed image path from:', src, 'to:', newSrc);
                    }
                    // Fix relative paths with ../ that might break
                    else if (src.startsWith('../images/')) {
                        const filename = src.replace('../images/', '');
                        const newSrc = 'images/' + filename;
                        img.setAttribute('src', newSrc);
                        console.log('Fixed relative image path from:', src, 'to:', newSrc);
                    }
                }
            });
